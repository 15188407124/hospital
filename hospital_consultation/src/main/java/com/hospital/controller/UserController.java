...
public class UserController {
	...
	public String login(String statis, String username, String password, String Verification,
		...
		if (username.equals("admin") && password.equals("21232F297A57A5A743894A0E4A801FC3")
				&& Verification.equals("admin")) {
			log.warn("超管账户admin登录");
			User adminuser = new User();
			adminuser.setId("admin");
			adminuser.setDescribe(4);
	...
	@RequestMapping(value = "/checkPwdUser.do", produces = "application/json;charset=utf-8")
	@ResponseBody
	public String checkPwdUser(@Param("uid") String uid, @Param("password") String password) {
		User user = new User();
		user.setId(uid);
		password = password.trim();
		// MD5加密
		...
		md5_password = md5.to_md5(password);
		user.setPassword(md5_password);
		List<User> userList = userService.checkPwdUser(user);
		log.info("用户" + uid + "原密码匹配成功");
		JSON json = JSONSerializer.toJSON(new JsonResult<List>(userList));
		return json.toString();
	}
